...
public class ParallaxListView extends ListView{
    ...
    protected boolean overScrollBy(int deltaX, int deltaY, int scrollX,
        ...
        if (isTouchEvent && deltaY < 0) {
            ...
            if (mImage.getHeight() <= drawableHeight) {
                int newHeight = (int) (mImage.getHeight() + Math
                        .abs(deltaY / 3.0f));
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                ResetAnimation animation = new ResetAnimation(mImage, startHeight,
                        endHeight);
    ...
    private void valueAnimator(final int startHeight, final int endHeight) {
        ...
        mValueAnim.setInterpolator(new OvershootInterpolator());// 差值器
